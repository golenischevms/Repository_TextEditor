    ...
    , ui(new Ui::MainWindow)
{
    ...
    setWindowTitle("Без имени - Текстовый редактор");
...
{
    QSettings setting("ncyxie", "Текстовый редактор");
...
void MainWindow::on_textEdit_textChanged()
{
    ...
    if (fileText != ui->textEdit->toPlainText())
    {
        if (isFresh == false)
        {
            setWindowTitle("*" + currentFile + " - Текстовый редактор");
        }
        else
        {
            setWindowTitle("*Без имени - Текстовый редактор");
    ...
    else
    {
        if (isFresh == false)
        {
            this->setWindowTitle(currentFile + " - Текстовый редактор");
        }
        else
        {
            this->setWindowTitle("Untitled - Текстовый редактор");
        }
    }

    if (wordsOn == true)
    {
        wordCountLabel->setText(tr("Слова: ") + QString::number(ui->textEdit->toPlainText().split(QRegularExpression("(\\s|\\n|\\r)+"), Qt::SkipEmptyParts).count()));
    }

    if (charOn == true)
    {
        charCountLabel->setText(tr("Символы: ") + QString::number(ui->textEdit->document()->characterCount() - 1));
    }
    ...
    if (linesOn == true)
    {
        ...
        linesCountLabel->setText(tr("Строки: ") + QString::number(ui->textEdit->document()->lineCount()) + flags);
    }
...
void MainWindow::closeEvent (QCloseEvent *event)
{
    if (fileText != ui->textEdit->toPlainText())
    {
        if (isFresh == false)
        {
            QMessageBox::StandardButton resBtn = QMessageBox::question(this, "Выход", "Хотите сохранить изменения ? " + currentFile + "?\n",
        ...
        {
            QMessageBox::StandardButton resBtn = QMessageBox::question(this, "Выход", "Хотите сохранить изменения в Без имени?\n",
                                                                    QMessageBox::No | QMessageBox::Yes | QMessageBox::Cancel,
                                                                    QMessageBox::Cancel);
...
void MainWindow::on_action_New_triggered()
{
    if (fileText != ui->textEdit->toPlainText())
    {
        if (isFresh == false)
        {
            QMessageBox::StandardButton resBtn = QMessageBox::question(this, "Выход", "Хотите сохранить изменения в " + currentFile + "?\n",
                                                                    QMessageBox::No | QMessageBox::Yes | QMessageBox::Cancel,
                                                                    QMessageBox::Cancel);
            ...
            if (resBtn == QMessageBox::No) {
                ...
                this->setWindowTitle("Без имени - Текстовый редактор");
                isFresh = true;
                fileText = ui->textEdit->toPlainText();
            } else if (resBtn == QMessageBox::Cancel) {
                return;
        ...
        else
        {
            ...
            if (resBtn == QMessageBox::No) {
                currentFile.clear();
                ui->textEdit->setText(QString());
                this->setWindowTitle("Без имени - Тесктовый редактор");
                isFresh = true;
                fileText = ui->textEdit->toPlainText();
            } else if (resBtn == QMessageBox::Cancel) {
                return;
...
void MainWindow::on_action_Open_triggered()
{
    ...
    if (!file.open(QIODevice::ReadOnly | QFile::Text))
    {
        ...
        errorMessage.setText("Текстовый редактор не смог открыть файл.");
        errorMessage.setInformativeText("Проверьте файл и попробуйте еще раз.");
        errorMessage.exec();
        return;
    }
    QTextStream in(&file);
    ...
    while(!in.atEnd())
    {
        ...
        if(in.status() != QTextStream::Ok)
        {
            ...
            errorMessage.setText("Неожиданная ошибка при открытии файла.");
            errorMessage.setInformativeText("Проверьте файл и попробуйте еще раз.");
            errorMessage.exec();
            file.close();
            return;
    ...
    ui->textEdit->document()->toPlainText();
    setWindowTitle(QFileInfo(file.fileName()).fileName() + " - Текстовый редактор");
...
void MainWindow::on_action_Save_triggered()
{
    ...
    if (!file.open(QIODevice::WriteOnly | QFile::Text))
    {
        ...
        errorMessage.setWindowTitle("Ошибка");
        errorMessage.setIcon(QMessageBox::Warning);
        errorMessage.setText("Текстовый редактор не смог сохранить изменения");
        errorMessage.setInformativeText("Убедитесь, что этот файл существует и у него есть разрешение на запись.");
        errorMessage.exec();
        return;
    }
    ...
    {
        {
            ...
            errorMessage.setWindowTitle("Error");
            errorMessage.setIcon(QMessageBox::Critical);
            errorMessage.setText("Непредвиденная ошибка при сохранении изменений.");
            errorMessage.setInformativeText("Проверьте файл и попробуйте еще раз.");
            errorMessage.exec();
        }
        file.cancelWriting();
...
{
    QPrinter printer;
    printer.setPrinterName("Имя принтера");
...
{
    QString link = "https://github.com/golenischevms/Repository_TextEditor.git";
...
void MainWindow::on_action_Search_with_Yandex_triggered()
{
    QTextCursor cursor= ui->textEdit->textCursor();
        if(cursor.hasSelection()) {
            QString link = "https://yandex.ru/search/?clid=2" + cursor.selectedText();
            QDesktopServices::openUrl(QUrl(link));
        }
}
...
void MainWindow::on_action_New_Window_triggered()
{
    ...
    newWindow->setWindowTitle("Без имени - Текстовый редактор");
